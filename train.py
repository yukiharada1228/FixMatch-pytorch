...
def main():
    ...
    parser.add_argument(
        "--arch",
        default="cifar_resnet34",
        type=str,
        choices=["wideresnet", "resnext", "cifar_resnet34"],
        help="dataset name",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--use-projector", action="store_true", default=False, help="use Projector head"
    )
    ...
    def create_model(args):
        ...
        elif args.arch == "cifar_resnet34":
            import models.resnet as models

            model = models.cifar_resnet34(num_classes=args.num_classes)
        ...
        if args.use_projector:
            ...
            model.fc = nn.Sequential(
                nn.Linear(dim_mlp, dim_mlp), nn.Dropout(p=0.7), nn.ReLU(), model.fc
            )
        logger.info(
...
def train(
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            try:
                # (inputs_u_w, inputs_u_s), labels_u = unlabeled_iter.next()
                # error occurs ↓
                (inputs_u_w, inputs_u_s), labels_u = next(unlabeled_iter)
            except:
                if args.world_size > 1:
                    unlabeled_epoch += 1
                    unlabeled_trainloader.sampler.set_epoch(unlabeled_epoch)
            ...
            if mask.sum().item() > 0:
                filtered_pseudo_label = pseudo_label[mask == 1]
                filtered_labels_u = labels_u[mask == 1]
                prec1, _ = accuracy(
                    filtered_pseudo_label, filtered_labels_u, topk=(1, 5)
                )
            else:
                prec1, _ = torch.Tensor([100.0]), torch.Tensor([100.0])
            ...
            pseudo_acc.update(prec1.item(), logits_u_s.shape[0])
            mask_probs.update(mask.mean().item())
            if not args.no_progress:
                p_bar.set_description(
                    "Train Epoch: {epoch}/{epochs:4}. Iter: {batch:4}/{iter:4}. LR: {lr:.4f}. Data: {data:.3f}s. Batch: {bt:.3f}s. Loss: {loss:.4f}. Loss_x: {loss_x:.4f}. Loss_u: {loss_u:.4f}. Mask: {mask:.2f}. PseudoAcc: {pseudo_acc:4f}.".format(
                        epoch=epoch + 1,
                        epochs=args.epochs,
                        batch=batch_idx + 1,
                        iter=args.eval_step,
        ...
        if args.local_rank in [-1, 0]:
            ...
            args.writer.add_scalar("train/5.pseudo_acc", pseudo_acc.avg, epoch)
